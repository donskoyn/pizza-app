...
const PizzaBlock = ({ id, imageUrl, name, types, sizes, price, category, rating }) => {
    ...
    const changeActiveType = (index) => setActtiveIndex(index);

    const avilableSizes = [26, 30, 40]
    const [activeSize, setactiveSize] = useState(null);
    const changeActiveSize = (index) => setactiveSize(index);
    return (
                ...
                <ul>
                    {
                        types.length === 1 ?
                            ...
                            :
                            arrTypePizza.map((typePizza, index) => {
                                return <li key={`${id}_${index}`} onClick={() => changeActiveType(index)} className={activeIndex === index ? "active" : ''}>{typePizza}</li>
                            })
                    }
