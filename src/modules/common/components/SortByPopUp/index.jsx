...
const SortByPopUp = () => {
    ...
    const [activeSort, setActiveSort] = useState({ name: 'popularity', index: 0 });
    const sortRef = useRef();
    const changeActivePopUp = () => setOpenPopUp(!openPopUp);
    const changeActiveSortObj = (name, index) => {
        setActiveSort({ name, index });
    ...
    return (
        <div ref={sortRef} className="sort">
            <div className="sort__label">
                ...
                <b>Sort by:</b>
                <span onClick={changeActivePopUp}>{activeSort.name}</span>
            </div>
                    ...
                    {sortName.map((name, i) => {
                        return (
                            <li key={`${name}_${i}`}
                                ...
                                className={activeSort.name === name && activeSort.index === i ? 'active' : ''}>
                                {name}
                            </li>
                        )
                    })}
