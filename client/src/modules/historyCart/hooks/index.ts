...
export const useHistoryCartArray = () => {
    ...
    const subDate = userDataCart.map((element: layoutHistoryInterface) => subDays(new Date(element?.date), 0));
    const [filteredDate, setFilteredDate] = useState<layoutHistoryInterface[]>([]);

    useEffect(() => {
        const filterCart = userDataCart?.filter((elem: layoutHistoryInterface) => moment(elem.date).format('L') === moment(startDate).format('L'));
